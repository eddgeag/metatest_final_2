...
## NULL = utilizar todos los factores disponibles en cada modelo MOFA.
## También puede definirse, por ejemplo, como c(1L, 2L).
MOFA_FACTORS_REQUESTED <- NULL

REF_CLASS <- "NP"

K_CV              <- 3L
CV_SEEDS          <- c(123L, 321L, 456L, 789L, 1001L)

BRMS_ITER_CV      <- 2000L
BRMS_CHAINS_CV    <- 2L

BRMS_ITER_TEST    <- 4000L
BRMS_CHAINS_TEST  <- 2L
...
##
##   1) menor variabilidad del LogLoss
##   2) mayor Balanced Accuracy media
##   3) mayor AUC media
##   4) menor LogLoss medio
##   5) menor número de variables p_g solo como desempate final
##
## En la CV repetida se añaden además:
##
##   - frecuencia de pertenencia al conjunto 1-SE;
##   - frecuencia de selección entre repeticiones.
##
## El tamaño p no gobierna la selección principal.
...
evaluate_brms_cv_grid <- function(return.list,
  ...
  cv_summary <- cv_results %>%
    ...
    dplyr::summarise(
      ...
      p = dplyr::first(p),
      p_target = dplyr::first(p_target),
      p_actual = dplyr::first(p_actual),
  ...
  ## 2) define el conjunto 1-SE:
  ##      mean_LogLoss <= min_LogLoss + SE_del_mejor;
  ## 3) dentro de 1-SE, prioriza menor variabilidad del LogLoss;
  ## 4) después mayor Balanced Accuracy;
  ## 5) después mayor AUC;
  ## 6) después menor LogLoss;
  ## 7) menor p solo como desempate final.
  ...
  cv_summary <- cv_summary %>%
    dplyr::mutate(
      ...
      selection_rule = dplyr::case_when(
        selected_by_rule ~ "hierarchical_1se_sd_balacc_auc_logloss_tie_p",
...
## ============================================================
## ESTABILIDAD CV REPETIDA SOBRE TODAS LAS GRIDS
## ------------------------------------------------------------
## - La primera ejecución CV_BRM se reutiliza.
## - Las semillas adicionales vuelven a evaluar todas las grids.
## - El conjunto 1-SE final se calcula sobre la CV repetida.
## - El tamaño p solo se usa como desempate final.
## ============================================================

evaluate_cv_stability_all_grids <- function(
    CV_BRM,
    return.list,
    thr_grid,
    ref = "NP",
  ...
  cv_seeds <- unique(as.integer(cv_seeds))
  initial_seed <- as.integer(initial_seed)
  
  if (!initial_seed %in% cv_seeds) {
    cv_seeds <- c(initial_seed, cv_seeds)
  }
  
  ## ----------------------------------------------------------
  ## 1) Reutilizar la CV inicial
  ## ----------------------------------------------------------
  
  initial_summary <- CV_BRM$cv_summary %>%
    dplyr::mutate(
      OrigGridID = GridID,
      repeat_id = 1L,
      cv_seed = initial_seed,
      selected_in_repeat = selected_by_rule %in% TRUE,
      within_1se_repeat = within_1se %in% TRUE
    )
  
  ## ----------------------------------------------------------
  ## 2) Ejecutar semillas restantes sobre TODAS las grids
  ## ----------------------------------------------------------
  
  additional_seeds <- setdiff(
    cv_seeds,
    initial_seed
  )
  
  additional_results <- lapply(
    seq_along(additional_seeds),
    function(ii) {
      
      current_seed <- additional_seeds[ii]
  ...
  cv_repeated_summary <- dplyr::bind_rows(
    c(
      list(initial_summary),
      additional_results
    )
  )
  
  ## ----------------------------------------------------------
  ## 3) Resumen de estabilidad por grid
  ## ----------------------------------------------------------
  
  initial_audit <- CV_BRM$cv_summary %>%
    dplyr::transmute(
      OrigGridID = GridID,
      ...
      initial_within_1se = within_1se,
      initial_selected_by_rule = selected_by_rule
    )
  
  stability_summary <- cv_repeated_summary %>%
    dplyr::group_by(OrigGridID) %>%
    dplyr::summarise(
      n_repeats = dplyr::n_distinct(cv_seed),
  ...
  ## ----------------------------------------------------------
  ## 4) Regla 1-SE basada en todas las repeticiones
  ## ----------------------------------------------------------
  
  best_mean_idx <- which.min(
    stability_summary$mean_cv_LogLoss
  )
  
  best_repeated_logloss <-
    stability_summary$mean_cv_LogLoss[best_mean_idx]
  
  best_repeated_se <-
    stability_summary$se_cv_LogLoss[best_mean_idx]
  
  if (!is.finite(best_repeated_se)) {
    best_repeated_se <- 0
  }
  
  repeated_1se_cutoff <-
    best_repeated_logloss + best_repeated_se
  ...
  stability_summary <- stability_summary %>%
    dplyr::mutate(
      ...
      within_repeated_1se =
        is.finite(mean_cv_LogLoss) &
        mean_cv_LogLoss <= repeated_1se_cutoff
    )
  
  repeated_candidates <- stability_summary %>%
    dplyr::filter(
      within_repeated_1se
    )
  
  if (nrow(repeated_candidates) == 0L) {
    warning(
      "No hubo grids dentro del conjunto 1-SE repetido. ",
      "Se utilizará la de menor LogLoss repetido."
    )
    
    repeated_candidates <- stability_summary %>%
      dplyr::slice(best_mean_idx)
  }
  ...
  ## Dentro de 1-SE:
  ## 1) menor variabilidad del LogLoss;
  ## 2) mayor frecuencia dentro de 1-SE;
  ## 3) mayor frecuencia de selección;
  ...
  best_stable <- repeated_candidates %>%
    dplyr::arrange(
      sd_cv_LogLoss,
      dplyr::desc(within_1se_freq),
  ...
  best_id <- best_stable$OrigGridID[1]
  ...
  stability_summary <- stability_summary %>%
    dplyr::mutate(
      ...
      final_selection_rule =
        dplyr::case_when(
          selected_final ~
            "repeated_1se_tie_sd_frequency_balacc_auc",
          
          within_repeated_1se ~
            "within_repeated_1se",
          
          TRUE ~
            "outside_repeated_1se"
        )
    ) %>%
    dplyr::arrange(
      dplyr::desc(selected_final),
      dplyr::desc(within_repeated_1se),
      sd_cv_LogLoss,
      dplyr::desc(within_1se_freq),
      dplyr::desc(selection_freq),
      dplyr::desc(mean_cv_BalAcc),
  ...
  best_stable <- stability_summary %>%
    dplyr::filter(selected_final) %>%
    dplyr::slice(1)
  
  list(
    candidate_ids =
      stability_summary %>%
      dplyr::filter(within_repeated_1se) %>%
      dplyr::pull(OrigGridID),
...
## ============================================================
## RESOLVER FACTORES MOFA DISPONIBLES
## NULL en MOFA_FACTORS_REQUESTED implica usar todos.
## ============================================================

.resolve_mofa_factors <- function(mod, requested = NULL) {
  ...
  while (is.list(F_ref) && length(F_ref) >= 1L) {
    F_ref <- F_ref[[1L]]
  }
  
  F_ref <- as.matrix(F_ref)
  
  if (ncol(F_ref) < 1L) {
    stop("El modelo MOFA no contiene factores utilizables.")
  }
  
  factor_names <- colnames(F_ref)
  
  if (
    !is.null(factor_names) &&
    all(grepl("^Factor[0-9]+$", factor_names))
  ) {
    available <- as.integer(sub("^Factor", "", factor_names))
  } else {
    available <- seq_len(ncol(F_ref))
  }
  ...
  requested <- sort(unique(as.integer(requested)))
  
  missing_factors <- setdiff(requested, available)
  
  if (length(missing_factors) > 0L) {
    stop(
      "Factores solicitados no disponibles en MOFA: ",
...
MOFA_FACTORS <- .resolve_mofa_factors(
  mod = mod,
  requested = MOFA_FACTORS_REQUESTED
)

cat("\n--- Factores MOFA resueltos dinámicamente ---\n")
print(MOFA_FACTORS)
...
extract_mofa_weights <- function(mod, rfm, view_map, factors = 1:2) {
  ...
  for (v in names(view_map)) {
    ...
    if (length(missing_factor_cols) > 0L) {
      stop(
        "No encuentro estos factores en los pesos MOFA de la vista ",
        v, ": ",
...
## ---------------------------------------------
## 4) Auditoría de pesos y dimensiones
## ---------------------------------------------
...
## Score MOFA principal por feature:
##
##   score_j = sum_f |loading_jf| * R2_f,vista
##
## La frecuencia de selección en grids se conserva únicamente
## como score descriptivo secundario y no selecciona el modelo.
...
## ============================================================
## 7) Inicializar indicadores
## Se completarán cuando existan el grid y el BRMS finales
## ============================================================
...
.select_mofa_mrmr_view <- function(score_v,
  ...
  ## Backfill no redundante:
  ##
  ## Una candidata con |rho| > rho_cutoff no se reintroduce.
  ## El algoritmo continúa recorriendo features de menor ranking MOFA
  ## hasta alcanzar n_keep con candidatas no redundantes.
  ##
  ## La vista solo quedará con menos de n_keep si se agotan todas
  ## las candidatas disponibles que cumplen el criterio de redundancia.
    ...
    for (j in seq_along(view_map)) {
      ...
      selected_v <- mrmr_i$selected %>%
        dplyr::mutate(
          ...
          mrmr_mode = "hard_cutoff_with_nonredundant_backfill",
          mrmr_backfill_nonredundant = TRUE,
...
## ------------------------------------------------------------
## thr_grid:
##   asignación solicitada antes del control de redundancia.
##
## thr_grid_model:
##   número efectivo de variables que realmente entra en BRMS.
## ============================================================

p_after_mrmr_vec <- vapply(
  return.list$train,
  ncol,
  integer(1)
)

if (length(p_after_mrmr_vec) != nrow(thr_grid)) {
  stop(
    "El número de matrices en return.list$train no coincide ",
    "con el número de filas de thr_grid."
  )
}

post_mrmr_counts_by_view <- score_grid_feature_selection_tbl %>%
  dplyr::count(
    GridID,
    view_code,
    name = "n_model"
  ...
  tidyr::pivot_wider(
    names_from = view_code,
    values_from = n_model,
    names_prefix = "n_model_",
    values_fill = 0L
  )

thr_grid_model <- thr_grid %>%
  dplyr::mutate(
    GridID = dplyr::row_number(),
    
    ## Número solicitado antes de mRMR.
    p_actual_before_mrmr = p_actual,
    
    ## Número realmente contenido en return.list.
    p_after_mrmr = p_after_mrmr_vec[GridID],
    
    p_lost_by_mrmr =
      p_actual_before_mrmr - p_after_mrmr,
    
    prop_kept_after_mrmr =
      p_after_mrmr / p_actual_before_mrmr,
    
    ## Desde este punto, p_actual representa el modelo efectivo.
    p_actual = p_after_mrmr,
    
    mrmr_mode =
      "hard_cutoff_with_nonredundant_backfill",
    
    allocation_scope =
      "n_* y q_* describen la asignacion solicitada pre-mRMR",
    
    model_size_scope =
      "p_actual describe el tamaño efectivo post-mRMR"
  ) %>%
  dplyr::left_join(
    post_mrmr_counts_by_view,
    by = "GridID"
  )

## Verificación contra las matrices usadas por BRMS.
p_effective_check <- vapply(
  return.list$train,
  ncol,
  integer(1)
)
...
if (!all(
  thr_grid_model$p_actual == p_effective_check
)) {
  stop(
    "Inconsistencia entre thr_grid_model$p_actual ",
    "y el número de columnas de return.list$train."
  )
}

grid_size_after_mrmr_tbl <- thr_grid_model %>%
  dplyr::select(
    GridID,
    p_target,
    p_actual_before_mrmr,
    p_after_mrmr,
    p_lost_by_mrmr,
    prop_kept_after_mrmr,
    dplyr::starts_with("n_model_")
  )

cat("\n--- Tamaño efectivo de grids antes/después de mRMR ---\n")
.print_tbl(
  grid_size_after_mrmr_tbl,
  n = Inf
)

cat("\n--- Grid efectiva usada por BRMS ---\n")
.print_tbl(
  thr_grid_model,
  n = Inf
)
...
write.csv(
  thr_grid_model,
  out_table("thr_grid_effective_post_mrmr.csv"),
  row.names = FALSE
)

saveRDS(
  thr_grid_model,
  out_rds("thr_grid_effective_post_mrmr.rds")
)

## Carpeta de plots mRMR
...
gg_grid_size_after_mrmr <- ggplot(
  ...
  labs(
    ...
    subtitle = paste0(
      "Backfill no redundante: se omiten candidatas con |rho| > ",
      MRMR_RHO_CUTOFF,
      " y se continúa el ranking MOFA"
    ),
    x = "GridID",
...
gg_mrmr_lost <- mrmr_strict_size_audit_tbl %>%
  ...
  facet_wrap(~ view, scales = "free_y") +
  labs(
    title = "Auditoría mRMR: diferencia entre variables solicitadas y seleccionadas",
    subtitle = paste0(
      "Solo existe pérdida de tamaño si se agotan candidatas con |rho| <= ",
      MRMR_RHO_CUTOFF
    ),
    x = "GridID",
    y = "Variables solicitadas no completadas"
  ) +
  theme_minimal()
...
CV_BRM <- evaluate_brms_cv_grid(
  return.list = return.list,
  thr_grid    = thr_grid_model,
  ref         = ref,
  K           = K_cv,
  p_min       = min(thr_grid_model$p_actual),
  p_max       = max(thr_grid_model$p_actual),
  brms_iter   = BRMS_ITER_CV,
  brms_chains = BRMS_CHAINS_CV,
  cores       = cores_cv,
  seed        = SEED_GLOBAL
)
...
## ============================================================
## Estabilidad por CV repetida sobre TODAS las grids
## Selección final por estabilidad, NO por test
## ============================================================

CV_STABILITY <- evaluate_cv_stability_all_grids(
  CV_BRM        = CV_BRM,
  return.list   = return.list,
  thr_grid      = thr_grid_model,
  ref           = ref,
...
TEST_ALL_GRIDS <- evaluate_test_all_grids_brms(
  ...
  thr_grid = thr_grid_model,
  ref          = ref,
  brms_iter    = BRMS_ITER_TEST,
  brms_chains  = BRMS_CHAINS_TEST,
  cores        = cores_cv,
  seed         = SEED_GLOBAL
)
...
CV_TEST_AGREEMENT <- CV_STABILITY$stability_summary %>%
  ...
  dplyr::mutate(
    ...
    rank_cv_stability = dplyr::row_number(),
...
## Construcción de los datos finales usando el grid seleccionado por CV estable
X_train <- return.list$train[[grid_id]]
X_test  <- return.list$test[[grid_id]]
y_train <- stats::relevel(refactor_levels(return.list$y_train), ref = ref)
y_test  <- stats::relevel(refactor_levels(return.list$y_test),  ref = ref)
...
fit_brm <- brms::brm(
  ...
  family    = brms::categorical(),
  prior     = priors,
  chains    = BRMS_CHAINS_FINAL,
  iter      = BRMS_ITER_FINAL,
  warmup    = BRMS_WARMUP_FINAL,
  refresh   = 0,
  backend   = backend,
  control   = ctrl,
  cores     = BRMS_CORES_FINAL,
  seed      = SEED_GLOBAL,
  save_pars = brms::save_pars(all = TRUE)
)
...
## ============================================================
## PREDICCIÓN Y DIAGNÓSTICO DEL BRMS FINAL
## ============================================================

## Probabilidades predictivas y métricas
P <- brms_probs(
  fit_brm,
  df_test
)[, levels(df_test$y), drop = FALSE]

metrics_best_brm <- metrics_from_probs(
  P,
  df_test$y
)
...
cm_best_brm <- caret::confusionMatrix(
  pred_lab,
  df_test$y
)

## Resumen del ajuste: calcular una sola vez
fit_summary <- summary(fit_brm)

print(fit_summary)

diag <- list(
  rhat = max(
    fit_summary$fixed[, "Rhat"],
    na.rm = TRUE
  ),
  ess_bulk = min(
    fit_summary$fixed[, "Bulk_ESS"],
    na.rm = TRUE
  ),
  ess_tail = min(
    fit_summary$fixed[, "Tail_ESS"],
    na.rm = TRUE
  )
)
...
## ============================================================
## LOO ADAPTATIVO
## Primero calcula PSIS-LOO convencional.
## Si Pareto-k > 0.70, intenta moment matching.
## ============================================================

loo_brm_initial <- tryCatch(
  loo::loo(
    fit_brm,
    moment_match = FALSE
  ),
...
loo_brm <- loo_brm_initial

loo_moment_match_attempted <- FALSE
loo_moment_match_used <- FALSE
loo_moment_match_error <- NA_character_

if (!is.null(loo_brm_initial)) {
  
  pareto_k_initial <- loo::pareto_k_values(
    loo_brm_initial
  )
  
  needs_moment_match <- any(
    pareto_k_initial >
      PARETO_K_MOMENT_MATCH_THRESHOLD,
    na.rm = TRUE
  )
  
  if (needs_moment_match) {
    
    loo_moment_match_attempted <- TRUE
    
    loo_mm_result <- tryCatch(
      list(
        object = loo::loo(
          fit_brm,
          moment_match = TRUE
    ...
    if (!is.null(loo_mm_result$object)) {
      
      loo_brm <- loo_mm_result$object
      loo_moment_match_used <- TRUE
      
    } else {
      
      loo_moment_match_error <-
        loo_mm_result$error
      
      warning(
        "Moment matching no pudo completarse. ",
        "Se conserva el LOO inicial: ",
        loo_moment_match_error
      )
    }
  }
...
if (
  !is.null(waic_brm) &&
  !is.null(waic_brm$pointwise) &&
  "p_waic" %in% colnames(waic_brm$pointwise)
) {
  
  waic_n_p_gt_0_4 <- sum(
    waic_brm$pointwise[, "p_waic"] > 0.4,
    na.rm = TRUE
  )
  
} else {
  
  waic_n_p_gt_0_4 <- NA_integer_
}

waic_warning_note <- dplyr::case_when(
  is.null(waic_brm) ~
    "WAIC no pudo calcularse.",
  
  is.finite(waic_n_p_gt_0_4) &&
    waic_n_p_gt_0_4 > 0L ~
    paste0(
      "WAIC secundario: ",
      waic_n_p_gt_0_4,
      " observaciones presentan p_waic > 0.4; priorizar LOO."
    ),
  
  TRUE ~
    "WAIC secundario; no se detectaron observaciones con p_waic > 0.4."
)

## ============================================================
## PARETO-k DEL OBJETO LOO FINAL
## ============================================================
...
if (all(is.na(pareto_k))) {
  ...
  pareto_diag <- list(
    ...
    pareto_k_mean =
      mean(pareto_k, na.rm = TRUE),
    
    pareto_k_n_gt_0_7 =
      sum(pareto_k > 0.7, na.rm = TRUE),
    
    pareto_k_n_gt_1 =
      sum(pareto_k > 1, na.rm = TRUE)
  )
}

## ROC multiclase one-vs-all
lev <- levels(df_test$y)

roc_list <- lapply(lev, function(lv) {
...
df_metrics <- metrics_best_brm %>%
  mutate(
    ...
    waic_n_p_gt_0_4 = waic_n_p_gt_0_4,
    waic_warning_note = waic_warning_note,
    
    loo_moment_match_threshold =
      PARETO_K_MOMENT_MATCH_THRESHOLD,
    
    loo_moment_match_attempted =
      loo_moment_match_attempted,
    
    loo_moment_match_used =
      loo_moment_match_used,
...
selected_grid_config <- thr_grid_model %>%
...
run_config <- data.frame(
  ...
  grid_selection_rule =
    "repeated_1se_tie_sd_frequency_balacc_auc",
  single_repeat_grid_selection_rule =
    "1se_sd_balacc_auc_logloss_tie_p",
  ...
  K_cv = K_cv,
  cores_cv = cores_cv,
  brms_iter_cv = BRMS_ITER_CV,
  brms_chains_cv = BRMS_CHAINS_CV,
  brms_iter_final = BRMS_ITER_FINAL,
  brms_chains_final = BRMS_CHAINS_FINAL,
  
  brms_chains_test = BRMS_CHAINS_TEST,
  brms_iter_test = BRMS_ITER_TEST,
  brms_warmup_final = BRMS_WARMUP_FINAL,
  brms_cores_final = BRMS_CORES_FINAL,
  seed_global = SEED_GLOBAL,
  mofa_factors = paste(MOFA_FACTORS, collapse = ";"),
  
  mofa_factors_requested = if (
    is.null(MOFA_FACTORS_REQUESTED)
  ) {
    "all_available"
  } else {
    paste(MOFA_FACTORS_REQUESTED, collapse = ";")
  },
  
  grid_p_target =
    selected_grid_config$p_target,
  
  grid_p_before_mrmr =
    selected_grid_config$p_actual_before_mrmr,
  
  grid_p_after_mrmr =
    selected_grid_config$p_after_mrmr,
  
  grid_p_lost_by_mrmr =
    selected_grid_config$p_lost_by_mrmr,
  
  cv_nesting_scope =
    paste0(
      "BRMS se reajusta dentro de cada fold; ",
      "MOFA, score MOFA y mRMR permanecen fijados ",
      "a partir del TRAIN completo."
    ),
...
if (is.null(dim(cm_best_brm$byClass))) {
  
  stats_class <- as.data.frame(
    t(cm_best_brm$byClass),
    check.names = FALSE
  )
  
  stats_class <- tibble::rownames_to_column(
    stats_class,
    "class"
  )
...
saveRDS(
  mofa_candidate_score_tbl,
  out_rds("mofa_candidate_score_tbl_final.rds")
)
